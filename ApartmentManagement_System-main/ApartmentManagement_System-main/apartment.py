...
def reset_maintenance(root, bhk2amt, bhk3amt, penalty):
    ...
    for block in block_details:
        ...
        # ✅ Only proceed if it's actually a list
        if isinstance(house_list, list):
            for house in house_list:
                if isinstance(house, list) and len(house) >= 7:
                    if house[2] == "Occupied":
                        pending = int(house[6][1])
...
def search_unoccupied_houses(root, bhk):
    ...
    def traverse(node):
        if node:
            ...
            # ✅ Fix: Only loop if it's actually a list
            if isinstance(house_list, list):
                for house in house_list:
                    if isinstance(house, list) and len(house) >= 3:
                        if house[1] == bhk and house[2].lower() == "unoccupied":
                            unoccupied_houses.append((block_name, house[0]))
...
def get_defaulters():
    ...
    def traverse(node):
        if node:
            ...
            block = node.item  # This is [block_number, block_name, house_list]
            block_name = block[1]
            house_list = block[2]

            # Ensure house_list is actually a list
            if isinstance(house_list, list):
                for house in house_list:
                    if isinstance(house, list) and len(house) >= 7:
                        if house[6][0] == "Unpaid":
...
@app.route('/delete_block', methods=['GET', 'POST'])
def delete_block():
    r = load_tree()
    if request.method == 'POST':
        block_name = request.form['block_name']
        ...
        block_details = generate_all_block_details(r)
        updated_blocks = [block for block in block_details if block[1] != block_name]

        if len(updated_blocks) == len(block_details):
            # Block not found
            return render_template('delete_block_error.html', error="Block not found!")

        # Rebuild tree and save
        new_root = create_tree(updated_blocks) if updated_blocks else None
        save_tree(new_root)

        # return render_template('delete_block_success.html', block_name=block_name)

    return render_template('delete_block.html')
